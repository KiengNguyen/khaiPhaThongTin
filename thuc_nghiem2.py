...
data_dir = 'data/vnexpress_tap_kiem_thu/'
texts, labels = [], []
...
doc2vec_model = Doc2Vec([TaggedDocument(words=t.split(), tags=[str(i)]) for i, t in enumerate(X_train)],
                        vector_size=400, window=5, min_count=1, workers=4)
...
# In kết quả đánh giá
print(f"🎯 KNN Precision: {k_precision:.4f}, Recall: {k_recall:.4f}, F1-Score: {k_f1:.4f}")
print(f"🎯 Naïve Bayes Precision: {nb_precision:.4f}, Recall: {nb_recall:.4f}, F1-Score: {nb_f1:.4f}")
print(f"🎯 Rocchio Precision: {r_precision:.4f}, Recall: {r_recall:.4f}, F1-Score: {r_f1:.4f}")
